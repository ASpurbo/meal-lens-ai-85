...
export function AppLayout({
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data
      } = await supabase.from("user_streaks").select("current_streak").eq("user_id", user.id).single();
      return data;
    },
    enabled: !!user?.id
  });
  ...
  if (!user) return null;
  return <div className="h-full flex flex-col overflow-hidden bg-gradient-to-b from-muted/30 to-background">
      {/* Header */}
      ...
      {/* Main Content */}
      <main className={cn("flex-1", hideMainScroll ? "overflow-hidden" : "overflow-y-auto overscroll-none hide-scrollbar")}>
        <div className={cn("px-5", hideMainScroll ? "h-full py-4" : "py-2 pb-28")}>
          {children}
        </div>
      </main>
      ...
      <nav className="flex-shrink-0 bg-background/80 backdrop-blur-lg border-t border-border/50 pb-safe relative">
        <div className="px-6">
          <div className="flex items-center justify-around py-3">
            ...
            return <Link key={item.path} to={item.path} className="flex flex-col items-center gap-1 py-1 px-6">
                  <item.icon className={cn("w-6 h-6 transition-colors", isActive ? "text-foreground" : "text-muted-foreground")} />
